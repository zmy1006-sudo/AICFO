...
import { ChevronLeft, ChevronRight, AlertTriangle, AlertCircle, Info, Calculator, X, Flame } from 'lucide-react';
    ...
    <div style={{ minHeight: '100dvh', backgroundColor: '#F7F7F7' }}>
      ...
      <div style={{ backgroundColor: '#FFFFFF', padding: '16px 16px 12px', borderBottom: '1px solid #EDEDED' }}>
        ...
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-3 rounded-xl text-white text-sm font-medium mt-2"
          style={{ background: 'linear-gradient(135deg, #07C160 0%, #06AD56 100%)', boxShadow: '0 2px 12px rgba(7,193,96,0.3)' }}
        >
          <div className="flex items-center gap-2">
            <Flame size={18} />
            <span>一键算税</span>
          </div>
          <span className="text-xs opacity-80">本月税费预览 ›</span>
        </button>
      </div>

      {/* 星期 */}
      <div style={{ backgroundColor: '#FFFFFF', padding: '12px 16px 4px' }}>
